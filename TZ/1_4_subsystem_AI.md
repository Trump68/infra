...
- **online-пайплайн обработки запроса** - гибридное извлечение, фильтрация, реранкинг и передача контекста в Generator.
...
- `merge-service` объединяет кандидатов (RRF), далее `filter-service` и `reranker-service` уточняют релевантность;
...
sequenceDiagram
  ...
  RR-->>R: Финальный проверенный контекст + метаданные цитирования
  R->>G: Контекст для ответа
...
### 10.4.6. Этап формирования окончательного ответа для пользователя
...
| `reranker-service`  | Реранкер (Cross-Encoder)            | Уточнение порядка релевантности                                             | Кандидаты после фильтрации                                 | Финальный проверенный контекст                     | Этап 3: пост-поисковая обработка    |
...
| `llm-generator`     | Generator LLM (смежный модуль)      | Формирование ответа по проверенному контексту                               | Финальный контекст, метаданные источников                  | Ответ с обязательным цитированием                  | Этап 4: генерация ответа            |
