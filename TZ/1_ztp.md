...
Сводная трассировка «принцип -> механизм -> требования ТЗ» приведена [на рис. 7-1](#fig-principles-trace-7-1).
...
<a id="fig-principles-trace-7-1"></a>
...
**4. Client Credentials:** приложение аутентифицируется само (без пользователя); применим для сервис-сервис вызовов, не для входа пользователя. Детали аутентификации сервисов бэкенда между собой — п. 1.4.9 и [рис. 9-4](#fig-9-4).
Обоснование выбора потока для ИС «Фармадок» — п. 1.2.1; детали потока — п. 1.4.4 и [рис. 9-3](#fig-9-3).
...
3. Для получения токена веб-клиентом (фронтенд) принят поток **Authorization Code + PKCE** в рамках OpenID Connect: IdP поддерживает discovery, выдаёт JWT с утверждениями о пользователе и ролях; обмен кода на токен выполняется в BFF, браузер не получает долгоживущие секреты. Детали потока — п. 1.4.4 и [рис. 9-3](#fig-9-3).
...
**Рис. 9-2. Последовательность обработки запроса на API Gateway** <a id="fig-9-2"></a>
...
Детали процесса входа и выдачи токена — п. 1.4.4 ([рис. 9-3](#fig-9-3)); обоснование выбора IdP — п. 1.2 и п. 1.4.5.
...
6. При последующих запросах к функциональности Системы фронтенд обращается к BFF; BFF проксирует запросы в API Gateway с заголовком `Authorization: Bearer <token>`. Шлюз проверяет JWT и RBAC (последовательность обработки запроса на шлюзе — п. 1.3.2, [рис. 9-2](#fig-9-2)) и при успехе передаёт запрос на backend с контекстом авторизации. 
Секреты клиента (client_secret при наличии) и долгоживущие токены не попадают в браузер — это соответствует рекомендациям по безопасному использованию OAuth2/OIDC для фронтенда. Последовательность взаимодействия отражена на [рис. 9-3](#fig-9-3).

**Рис. 9-3. Поток входа и выдачи токена (Authorization Code + PKCE)** <a id="fig-9-3"></a>
...
Секреты между сервисами не передаются; единая точка выдачи и отзыва — IdP. Поток аутентификации сервисов показан на [рис. 9-4](#fig-9-4).

**Рис. 9-4. Аутентификация сервисов бэкенда (Client Credentials)** <a id="fig-9-4"></a>
...
Последовательность проверки JWT и RBAC на шлюзе — п. 1.3 и [рис. 9-2](#fig-9-2).
...
**Рис. 10.2-1. Логические потоки подсистемы наблюдаемости и аудита** <a id="fig-10-2-1"></a>
...
<a id="fig-observability-pipeline-10-5-1"></a>
...
<a id="fig-storage-scheme-11-2-1"></a>
...
flowchart LR
  subgraph SERVICE_LAYER[Сервисный слой]
    direction TB
    DOCS[Исходный документ] --> ING[Сервис приёма документов]
    ING --> MS[Сервис разметки]
    MS --> TS[Сервис тэгирования]
    TS --> CS[Сервис чанкинга]
    CS --> VI[Векторный индексатор]
    CS --> LI[Лексический индексатор]
  end
...
<a id="fig-offline-flow-13-3-1"></a>
...
sequenceDiagram
  ...
  participant CMP as Сервис сравнения
  participant IDX as Индекс/Хранилище
  participant REP as Сервис отчётов

  U->>UI: Выбор Документ A и Документ B
  UI->>API: POST /compare/runs
  API->>CMP: Запуск прогона сравнения
  CMP->>IDX: Получение структур и фрагментов
  IDX-->>CMP: Данные документов
  CMP->>CMP: Выравнивание и семантическое сравнение
  CMP-->>API: Список различий и критичность
  API->>REP: Формирование отчёта
  REP-->>API: reportId/ссылка
  API-->>UI: Результат сравнения + экспорт
  UI-->>U: Отображение различий
```

<a id="fig-compare-seq-13-5-1"></a>
...
```mermaid
flowchart TD
  start["Старт проверки"] --> upload["Загрузка документа и выбор профиля"]
  upload --> prep["Подготовка контекста требований"]
  prep --> check["Структурно-семантическая проверка"]
  check --> full{"Все обязательные\nтребования покрыты?"}
  full -->|"Да"| classify["Классификация замечаний"]
  full -->|"Нет"| gaps["Фиксация пропусков"]
  gaps --> classify
  classify --> critical{"Есть критичные\nнесоответствия?"}
  critical -->|"Да"| decision1["Статус: не соответствует"]
  critical -->|"Нет"| decision2["Статус: условно соответствует"]
  decision1 --> report["Формирование заключения и рекомендаций"]
  decision2 --> report
  report --> finish["Отчет + карта соответствия"]
```

<a id="fig-compliance-flow-13-6-1"></a>
...
Связь групп API с подсистемами и сценариями UI приведена [на рис. 14.8-1](#fig-api-map-14-8-1).
...
```mermaid
flowchart LR
  ui["UI-экраны (14.7)"] --> bff["Backend-сервис UI (BFF/API)"]
...
<a id="fig-api-map-14-8-1"></a>

**Рисунок 14.8-1.** Карта API backend-сервиса UI и его связи с подсистемами.
...
flowchart LR
  ...
  t413["ТЗ 4.1.3"] --> rC["Разделы 10, 11"]
  t414["ТЗ 4.1.4"] --> rD["Разделы 9, 10, 11, 14.5, 14.8"]
...
<a id="fig-traceability-15-0-1"></a>
