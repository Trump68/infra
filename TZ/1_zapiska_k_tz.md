...
#### 11.3.4. Redis-кэш (опционально на стадии разработки)
...
Сводная логическая схема взаимодействия компонентов приведена в [п. 5.2.7](#fig-logicheskaya-shema-5-2-7).
...
Укрупнённый поток выполнения конвейера (цикл оркестратора и модулей) показан [в п. 5.3.2](#fig-diagramma-potoka-5-3-2). Диаграмма последовательности обмена сообщениями между входной очередью, оркестратором, PostgreSQL, брокером, обработчиком и бакетом контекста в S3 приведена [в п. 5.3.3](#fig-posledovatelnosti-5-3-3).

#### 12.3.1. Механизм запуска обработчиков и обмен сообщениями

Взаимодействие участников во времени отражено [в п. 5.3.3](#fig-posledovatelnosti-5-3-3); ниже дано текстовое описание шагов и состава сообщений.
...
Критерии приёмки подсистемы RAG, сценарии тестирования и ограничения (область применения, границы ответственности компонентов) задаются ТЗ и согласуются с техническим проектом. В контексте настоящего документа критично: соответствие целевым показателям извлечения и времени ответа (п. 4.1.2 ТЗ); корректная работа RBAC на пути к Generator; воспроизводимость цитирования; согласованность индексов с актуальным корпусом после обновлений (п. 13.5).

---

### 13.5. Обновление индексов и согласованность
...
#### 13.5.1. Сводный список сервисов
...
#### 13.5.2. Сводный список хранилищ (БД и объектное хранилище)
...
3. **Связность с контуром безопасности** — отсутствие хранения долгоживущих секретов в браузере; работа с токенами и входом в соответствии с раздел 9 настоящей записки.
...
| ИИ и RAG | Отображение запросов к базе знаний, цитат и предупреждений о достоверности; не подменяет серверную валидацию (раздел 13 настоящей записки). |
